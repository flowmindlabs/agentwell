...
_REDACT_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"sk-ant-[A-Za-z0-9\-_]{32,}"), "sk-ant-***REDACTED***"),
    (re.compile(r"sk_live_[A-Za-z0-9]{24,}"), "sk_live_***REDACTED***"),
    (re.compile(r"sk_test_[A-Za-z0-9]{24,}"), "sk_test_***REDACTED***"),
    (re.compile(r"sk-[A-Za-z0-9]{32,}"), "sk-***REDACTED***"),
    (re.compile(r"(Bearer\s+)[A-Za-z0-9\-._~+/]{20,}", re.IGNORECASE), r"\1***REDACTED***"),
    (re.compile(r"(Token\s+)[A-Za-z0-9\-._~+/=]{16,}", re.IGNORECASE), r"\1***REDACTED***"),
    (re.compile(r"(Basic\s+)[A-Za-z0-9+/=]{16,}", re.IGNORECASE), r"\1***REDACTED***"),
    (re.compile(r"(Digest\s+)[A-Za-z0-9\-._~+/=]{16,}", re.IGNORECASE), r"\1***REDACTED***"),
    (re.compile(r"hooks\.slack\.com/services/[A-Za-z0-9/]+"), "hooks.slack.com/***REDACTED***"),
    (re.compile(r"(postgres|postgresql|mysql|mongodb|redis)://[^\s\"']+:[^\s\"'@]+@", re.IGNORECASE), r"\1://***REDACTED***@"),
